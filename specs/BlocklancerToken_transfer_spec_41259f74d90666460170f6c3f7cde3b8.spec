...
rule ValidateTransferEffectiveness() {
    address $sender;
    address $recipient;
    ...
    if ($sender != $recipient) {
        assert(balances[$sender] == $senderInitialBalance - $transferAmount);
        assert(balances[$recipient] == $recipientInitialBalance + $transferAmount);
    } else {
        assert(balances[$sender] == $senderInitialBalance);
    }
}}