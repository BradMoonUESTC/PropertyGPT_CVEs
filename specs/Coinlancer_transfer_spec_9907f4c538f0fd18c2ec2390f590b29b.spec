...
rule ValidateTransferIncreasesRecipient() {
    address $recipient;
    uint256 $transferAmount;
    uint256 initialBalanceRecipient = balances[$recipient];
    transfer($recipient, $transferAmount);

    assert(balances[$recipient] == initialBalanceRecipient + $transferAmount);
}}