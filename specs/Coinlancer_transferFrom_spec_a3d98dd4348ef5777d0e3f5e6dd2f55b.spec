...
rule ValidateTransferFromSuccess() {
    address $sender;
    address $from;
    address $to;
    uint256 $amount;
    ...
    uint256 allowanceBefore = allowed[$from][$sender];
    transferFrom($from, $to, $amount);

    if ($amount > 0 && balanceFromBefore >= $amount && allowanceBefore >= $amount
        && (balances[$to] + $amount > balances[$to])) {
        assert(balances[$from] == balanceFromBefore - $amount);
        assert(allowed[$from][$sender] == allowanceBefore - $amount);
        assert(balances[$to] == balanceToBefore + $amount);
    } else {
        assert(balances[$from] == balanceFromBefore);
        assert(allowed[$from][$sender] == allowanceBefore);
