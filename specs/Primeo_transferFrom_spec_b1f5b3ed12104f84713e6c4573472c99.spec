...
rule BalanceTransferMaintainsTotalSupply() {
    address $from;
    address $to;
    uint256 $amount;
    uint256 totalSupplyBefore = totalSupply;
    uint256 balanceOfFromBefore = balances[$from];
    uint256 balanceOfToBefore = balances[$to];
    ...
    assert(balances[$from] == balanceOfFromBefore - $amount);
    assert(balances[$to] == balanceOfToBefore + $amount);
