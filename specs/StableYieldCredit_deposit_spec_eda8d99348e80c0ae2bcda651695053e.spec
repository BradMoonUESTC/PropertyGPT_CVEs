...
rule VerifyLiquidityAndValueAfterDeposit() {
    IERC20 $token;
    uint256 $amount;
    uint256 $value;

    uint256 balanceBefore = collateral[msg.sender][address($token)];
    uint256 creditBefore = collateralCredit[msg.sender][address($token)];
    uint256 totalSupplyBefore = totalSupply;

    _deposit($token, $amount);

    uint256 balanceAfter = collateral[msg.sender][address($token)];
    uint256 creditAfter = collateralCredit[msg.sender][address($token)];
    uint256 totalSupplyAfter = totalSupply;

    assert((balanceAfter - balanceBefore) > 0);
    assert((creditAfter - creditBefore) == $value);
    assert((totalSupplyAfter - totalSupplyBefore) >= $value);
}}