...
rule TransferEnsuresBalanceIntegrity() {
    address $to;
    uint256 $amount;
    uint256 balanceSenderBefore = balances[msg.sender];
    uint256 balanceRecipientBefore = balances[$to];

    require($amount > 0);
    require(balanceSenderBefore >= $amount);

    transfer($to, $amount);

    if (msg.sender != $to) {
        assert(balances[msg.sender] == balanceSenderBefore - $amount);
        assert(balances[$to] == balanceRecipientBefore + $amount);
    } else {
        assert(balances[msg.sender] == balanceSenderBefore);
    }
}}