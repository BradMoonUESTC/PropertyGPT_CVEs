...
rule MaintainCollateralAndCreditPostDeposit() {
    address $user;
    IERC20 $token;
    uint $amount;
    uint $init_collateral = collateral[$user][address($token)];
    uint $init_credit = collateralCredit[$user][address($token)];
    uint $init_supply;

    _deposit($token, $amount);

    uint $final_collateral = collateral[$user][address($token)];
    uint $final_credit = collateralCredit[$user][address($token)];
    uint $final_supply = totalSupply;

    assert($final_collateral > $init_collateral);
    assert($final_credit > $init_credit);
    assert($final_supply > $init_supply);
}}