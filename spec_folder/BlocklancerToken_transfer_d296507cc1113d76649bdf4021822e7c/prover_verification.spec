...
rule SuccessfulTransferUpdatesBalances() {
    ...
    uint256 $init_receiver_balance;
    uint256 $amount;

    require($amount > 0);
    require($init_sender_balance >= $amount);
    
    balances[$sender] = $init_sender_balance;
    balances[$receiver] = $init_receiver_balance;

    transfer($receiver, $amount);

    assert(balances[$sender] == $init_sender_balance - $amount);
    assert(balances[$receiver] == $init_receiver_balance + $amount);
}}