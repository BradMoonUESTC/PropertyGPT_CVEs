...
rule ValidTransferSuccessCondition() {
    address $sender;
    address $recipient;
    uint256 $sendAmount;
    uint256 $senderInitialBalance = balances[$sender];
    uint256 $recipientInitialBalance;
    
    require($sendAmount > 0);
    require(balances[$sender] >= $sendAmount);

    transfer($recipient, $sendAmount);

    assert(balances[$sender] == $senderInitialBalance - $sendAmount);
    assert(balances[$recipient] == $recipientInitialBalance + $sendAmount);
}}