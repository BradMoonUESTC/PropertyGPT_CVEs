...
rule TransferFromChecks() {
    ...
    require($initialBalanceFrom >= $amount && $initialAllowance >= $amount && $amount > 0);
    ...
    if (success) {
        ...
        // Ensures state doesn't change if the operation was not successful.
        assert(balances[$from] == $initialBalanceFrom);
        assert(balances[$to] == $initialBalanceTo);
        assert(allowed[$from][msg.sender] == $initialAllowance);
    }
}}